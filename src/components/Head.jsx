...
const Head = () => {
  ...
  return (
    <div className="w-full grid justify-between items-center grid-flow-col p-4 mb-4 backdrop-blur-md bg-opacity-50">
      ...
      <div className="flex flex-col gap-2">
        <div className="flex">
          <input
            className="w-[60px] sm:w-[450px]  border border-black font-semibold text-1xl outline-red-300 rounded-l-full"
            type="text"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            onFocus={() => setShowSuggestionInView(true)}
            onBlur={() => setShowSuggestionInView(false)}
          />
        ...
        {showSuggestionInView && (
          <div className="fixed bg-white my-10 px-6 w-[448px]">
              ...
              {suggestions &&
                suggestions.map((suggestion) => {
                  return (
                    <li
                      key={suggestion}
                      className="hover:bg-slate-100 p-2 font-semibold"
                    >
                      {suggestion}
                    </li>
                  );
